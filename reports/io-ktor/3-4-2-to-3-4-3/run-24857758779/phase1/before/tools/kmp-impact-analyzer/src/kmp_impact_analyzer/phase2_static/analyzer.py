...
Two-layer architecture:
  Layer 1 (tree-sitter): import-based dependency graph + BFS propagation.
    Always runs, no compilation needed, <1 second.
  Layer 2 (KSP): resolved type references that enrich the graph.
    Runs when Gradle + Java are available, ~30-90 seconds.

Metrics are sourced from Detekt (when available) or a lightweight
heuristic.  Kover provides test coverage percentages.
"""
...
from .ksp_enricher import enrich_graph_with_ksp, run_ksp_enrichment
...
def run_static_analysis(manifest: ShadowManifest) -> ImpactGraph:
    ...
    # ── Layer 2: KSP type resolution (optional) ──
    ksp_data = run_ksp_enrichment(before_dir)
    ksp_edges_added = 0
    if ksp_data:
        ksp_edges_added = enrich_graph_with_ksp(
            graph._imports_from,
            graph._imported_by,
            ksp_data,
            str(before_dir),
    ...
    # ── BFS propagation (with optional Detekt/Kover metrics) ──
    ...
    result = ImpactGraph(
        ...
        ksp_edges_added=ksp_edges_added,
    )

    ksp_label = f", {ksp_edges_added} KSP type edges" if ksp_edges_added else ""
    log.info(
        f"[bold green]Phase 2 complete[/bold green]: "
        f"{result.total_impacted}/{result.total_project_files} files impacted "
        f"({len(seeds)} direct, {len(ea_resolver.pairs)} expect/actual pairs{ksp_label})"
    )
    return result
